...
import { ReligionResponse } from '@app/shared/models/base.interface';
declare var $: any;
...
export class ListComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  constructor(
    ...
    private religionlevelSvc: ReligionService,
  ...
  loadTable(): void {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        that.religionlevelSvc.getAll(dataTablesParameters).subscribe(resp => {
  ...
  onDelete(positionId: number): void {
    if (window.confirm('Do you really want remove this data')) {
      this.religionlevelSvc
        .delete(positionId)
        .pipe(takeUntil(this.destroy$))
        .subscribe((res: ReligionResponse) => {
          if (res.code === 201) {
            this.rerender();
          }
        });
