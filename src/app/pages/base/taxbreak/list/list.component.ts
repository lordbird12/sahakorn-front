...
import { TaxbreakResponse } from '@app/shared/models/base.interface';
declare var $: any;
...
export class ListComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  constructor(
    ...
    private taxbreaklevelSvc: TaxbreakService,
  ...
  loadTable(): void {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        that.taxbreaklevelSvc.getAll(dataTablesParameters).subscribe(resp => {
  ...
  onDelete(positionId: number): void {
    if (window.confirm('Do you really want remove this data')) {
      this.taxbreaklevelSvc
        .delete(positionId)
        .pipe(takeUntil(this.destroy$))
        .subscribe((res: TaxbreakResponse) => {
          if (res.code === 201) {
            this.rerender();
          }
        });
